    ...
    <div className="min-h-screen bg-secondary flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-headline">Selamat Datang, {user?.displayName || 'Pengguna Baru'}!</CardTitle>
            ...
            <div className="flex items-center justify-center gap-4 pt-4">
                <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold ${stage === 'wallets' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground border'}`}>1</div>
                    <span className={`font-semibold ${stage === 'wallets' ? 'text-primary' : 'text-muted-foreground'}`}>Atur Dompet</span>
                </div>
                <div className="h-0.5 w-16 bg-border"></div>
                 <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold ${stage === 'categories' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground border'}`}>2</div>
                    <span className={`font-semibold ${stage === 'categories' ? 'text-primary' : 'text-muted-foreground'}`}>Atur Kategori</span>
                </div>
            </div>
        </CardHeader>
        <Form {...form}>
            ...
            {stage === 'wallets' && (
                <div className="animate-in fade-in duration-300">
                    <CardContent className="space-y-4">
                        <h3 className="font-semibold text-lg">Langkah 1: Atur Dompet Anda</h3>
                    ...
                    <CardFooter>
                         <Button type="button" className="w-full" onClick={handleNextStep}>Lanjut <ArrowRight className="ml-2 h-4 w-4"/></Button>
                    </CardFooter>
                </div>
            )}
            {stage === 'categories' && (
                 <div className="animate-in fade-in duration-300">
                    <CardContent className="space-y-4">
                        <h3 className="font-semibold text-lg">Langkah 2: Konfirmasi Kategori</h3>
                    ...
                    <CardFooter className="grid grid-cols-2 gap-4">
                        <Button type="button" variant="outline" onClick={() => setStage('wallets')}>Kembali</Button>
                        <Button type="submit" disabled={isSubmitting}>{isSubmitting ? <Loader2 className="animate-spin" /> : <><Check className="mr-2 h-4 w-4"/>Simpan & Selesai</>}</Button>
                    </CardFooter>
                </div>
            )}
          </form>
